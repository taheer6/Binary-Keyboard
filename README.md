...
## Intro
The Binary Keyboard is exactly what it sounds like: an 8-bit ASCII keyboard that converts binary input into letters, numbers, and a few special characters like backspace, space, and return. It uses two switches to enter `0` and `1`, builds a full byte, shows the active 8-bit input on an LCD, and then sends the finished character to a computer as a USB keyboard. This is super useful for those who love binary and need practicle applications to utilize their disposable knowledge. For those who struggle with binary, don't fret! I'm building a keyboard to binary converter, all you need is a simple extension and once you plug the extension and a keyboard into the binary keyboard, it'll convert your strokes into binary using mechanical soleniods.
